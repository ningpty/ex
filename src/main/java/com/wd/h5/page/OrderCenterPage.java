...
// 删除地址按钮
    ...
    //-------------------------------------待发货相关
// 待发货无订单文案是否存在
    public boolean isWaitSendNoOrderStatus() throws Exception {
        return isElementExist(waitSendNoOrder);
    }
    
  //获取待发货列表订单上店铺名称
    Locator waitSendshopname = new Locator("#js_orderList > div:nth-child(3) > nav > h2 >a", ByType.cssSelector);
...
//  获取跳转页待发货店铺名称
    Locator waitSendshopTitle = new Locator("hd_name");
    public  String getWaitSendShopTitle() throws Exception{
        return getText(waitSendshopTitle);
    }
    //获取待发货店铺名称
...
// 获取待发货列表订单上商品名称
    Locator waitSendGoodsName = new Locator("#js_orderList > div:nth-child(3) > nav:nth-child(1) > nav > a > div > div.flex-auto.p-details > div > div.flex-auto > span", ByType.cssSelector);
    
//    点击待发货定案商品名称
    public void  clickwaitSendGoodName() throws Exception{
        click(waitSendGoodsName);
    }
//   获取待发货店铺的商品名称
    Locator waitSendGoodsTitle = new Locator("#o_ul > li > a > div > div.flex-auto.p-details > div > div.flex-auto > span", ByType.cssSelector);
    public  String getWaitSendGoodTitle() throws Exception{
        return getText(waitSendGoodsTitle);
    }
